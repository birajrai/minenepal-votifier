...
function createMessage(header: string, vote: Vote, options: Options): Buffer {
...
function _vote(options: Options, cb: Callback): void {
    ...
    socket.on('error', function (err: Error & { message?: string }) {
        done(new Error('Socket error: ' + (err && err.message ? err.message : String(err))));
    });
    ...
    socket.once('data', function (buf: Buffer) {
        ...
        socket.once('data', function (respBuf: Buffer) {
            try {
...
function vote(options: Options, cb?: Callback): Promise<void> | void {
    ...
    return _vote(options, cb as Callback);
}

export default vote;

// CommonJS compatibility for consumers using require()
